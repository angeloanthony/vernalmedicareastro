...
| 2026-08-26 | — | — | — | — | — | Confounder **#5** fired again (Prescription Assistance Batch 2, WORK-LOG 2026-08-26): +1 in-content inbound link to treated pages entresto, xarelto, repatha from their rebuilt assistance pages. **Control arm protected by design:** a new `/trelegy-assistance-program.html` shipped but does not link to the control page `does-medicare-cover-trelegy`, and `drugCoverage.ts` was not edited (Trelegy still has no `assistanceSlug`, so the control page still links to the hub, not the new page). Verified zero links in either direction in `dist/`. No sitewide template change (#4 did not fire). |
