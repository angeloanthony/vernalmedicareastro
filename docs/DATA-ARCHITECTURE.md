...
| `medicationAssistance/` | Prescription Assistance project (docs/PRESCRIPTION-ASSISTANCE-PROJECT.md): one independently-researched, source-dated record per medication (`farxiga.ts`, …); `index.ts` is the registry + the locked title/H1/URL pattern; `shared.ts` holds cross-cutting organisational facts; `drugClasses.ts` axis 2 of the taxonomy (`conditions` is axis 1, in `conditions.ts`); `categories.ts` the browse views derived from those two axes (never stored on a record) | `MedicationAssistanceRecord`, `AssistanceProgram`, `SourceRef` |
